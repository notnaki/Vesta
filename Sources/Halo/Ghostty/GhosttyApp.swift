...
final class GhosttyApp {
    ...
    /// halo-* keys we read ourselves from the config file text. libghostty
    ...
    private init() {
        ...
        // halo-* overrides. Build a LOCAL HaloConfig (not .shared — that reads
        ...
        if let s = hc.surface { t.background = s }   // halo-surface overrides ghostty background
        self.theme = t
        ...
        var runtime = ghostty_runtime_config_s(
            ...
            wakeup_cb: haloWakeupCB,
            action_cb: haloActionCB,
            read_clipboard_cb: haloReadClipboardCB,
            confirm_read_clipboard_cb: haloConfirmReadClipboardCB,
            write_clipboard_cb: haloWriteClipboardCB,
            close_surface_cb: haloCloseSurfaceCB
        )
...
private func haloWakeupCB(_ ud: UnsafeMutableRawPointer?) {
    GhosttyApp.wakeup(ud)
}
private func haloActionCB(_ app: ghostty_app_t?, _ target: ghostty_target_s, _ action: ghostty_action_s) -> Bool {
    GhosttyApp.action(app, target: target, action: action)
}
private func haloReadClipboardCB(_ ud: UnsafeMutableRawPointer?, _ loc: ghostty_clipboard_e, _ state: UnsafeMutableRawPointer?) -> Bool {
    GhosttyApp.readClipboard(ud, location: loc, state: state)
}
private func haloConfirmReadClipboardCB(_ ud: UnsafeMutableRawPointer?, _ str: UnsafePointer<CChar>?, _ state: UnsafeMutableRawPointer?, _ req: ghostty_clipboard_request_e) {
    GhosttyApp.confirmReadClipboard(ud, string: str, state: state, request: req)
}
private func haloWriteClipboardCB(_ ud: UnsafeMutableRawPointer?, _ loc: ghostty_clipboard_e, _ content: UnsafePointer<ghostty_clipboard_content_s>?, _ len: Int, _ confirm: Bool) {
    GhosttyApp.writeClipboard(ud, location: loc, content: content, len: len, confirm: confirm)
}
private func haloCloseSurfaceCB(_ ud: UnsafeMutableRawPointer?, _ alive: Bool) {
